...
struct Day14: Day {
    ...
    struct ReactionSet {
        ...
        func resolve() -> (minNeeded: Int, left: [String: Int]) {
            guard let targetReaction = reactions["FUEL"] else {
                fatalError("No FUEL to be produced ... ever :'(")
            }
            var left = [String: Int]()
            let min = targetReaction.reduce(with: reactions, productsLeft: &left)
            return (min, left)
        }

        func fuelProducedWithATrillionOre() -> Int {
            let amount = 1_000_000_000_000
            let (orePerFuel, leftForOne) = resolve()
            var fuel = amount / orePerFuel
            var left = leftForOne.mapValues { $0 * fuel }
            left["ORE"] = amount % orePerFuel
